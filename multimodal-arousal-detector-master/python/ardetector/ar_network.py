...
class ARModel(object):
    def __init__(self, config):
        ...
        # Placeholders
        self._features = tf.placeholder(tf.float32, [None, config.num_features, 1], name='ModelInput')
        self._targets = tf.placeholder(tf.float32, [None, config.num_classes], name='ModelOutput')
        self._mask = tf.placeholder(tf.float32, [None, int(config.num_classes/2)], name='ModelMask')
        self._batch_size = tf.placeholder(tf.int32, name='BatchSize')
        self._step = tf.placeholder(tf.float32, [])
        ...
        with tf.variable_scope('eeg_resnet') as scope:
            print('eeg resnet: ')
            hidden_eeg = models.resnet(eeg,config.num_resnet,1)
        with tf.variable_scope('eog_resnet') as scope:
            print('eog resnet: ')
            hidden_eog = models.resnet(eog,config.num_resnet,2)
        with tf.variable_scope('emg_resnet') as scope:
            print('emg resnet: ')
        ...
        with tf.variable_scope('blstm') as scope:
            # Forward cells
            cell_fw = tf.nn.rnn_cell.BasicLSTMCell(config.num_hidden, forget_bias=1.0,state_is_tuple=True)
            cell_fw = tf.nn.rnn_cell.DropoutWrapper(cell_fw, input_keep_prob=iKeepProb, output_keep_prob=oKeepProb)
            # Backward cells
            cell_bw = tf.nn.rnn_cell.BasicLSTMCell(config.num_hidden, forget_bias=1.0,state_is_tuple=True)
            cell_bw = tf.nn.rnn_cell.DropoutWrapper(cell_bw, input_keep_prob=iKeepProb, output_keep_prob=oKeepProb)
            #initial_state = tf.nn.rnn_cell.LSTMStateTuple(self._initial_state[:,:config.num_hidden],self._initial_state[:,config.num_hidden:])
            outputs,final_state = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, hidden_concat, dtype=tf.float32)#, initial_state = initial_state)
        ...
        with tf.variable_scope('hidden_reduce') as scope:
        ...
        tf.summary.scalar('Cross_ent',self._cross_ent)
        tf.summary.scalar('Learning Rate',self._learning_rate)
        tf.summary.scalar('Acuracy_w',self._accuracy_w)
    ...
    def intelligent_cost(self, logits):
        ...
        cross_ent_ar = tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits[:,0:2], labels=self._targets[:,0:2])
        cross_ent_w  = tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits[:,2:], labels=self._targets[:,2:])
