...
for i = 1:size(data,1)
    ...
    dataEnd = ceil(hdr.fs(i)/max_fs*size(data,2));           % int of the size/end index of data
    dataClip = data(i,1:dataEnd);                                    % data of one channel
    % Check up/down sample factor
    [p,q] = rat(des_fs/hdr.fs(i));
    if hdr.fs(i) ~= des_fs
        % Resample
        dataResample = resample(dataClip,p,q);
        dataR(i,1:ceil(dataEnd*p/q)) = dataResample;
    else
