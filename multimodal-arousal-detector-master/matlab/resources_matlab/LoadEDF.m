...
if ~exist('ftype','var')
    elseif contains(p_file, 'mni')
        ftype = 'mni';
    elseif contains(p_file,'mros')
        ftype = 'mros';
    elseif contains(p_file,'cfs')
        ftype = 'cfs';
end
% Load data
[hdr,data] = edfread(p_file, 'targetSignals', evalin('base', 'list_signals')); % TODO: load only specific channels because Remote Desktop crashes when I try to load all channels. testing_script.m has an example of this.

switch ftype
    case 'mni'      
        % Get signal derivations
        
        A1 = contains(hdr.label,'M1');
        A2 = contains(hdr.label,'M2');
        C3 = contains(hdr.label,'C3');        
        EMG1 = contains(hdr.label,'RFlex');
        EMG2 = contains(hdr.label,'LFlex');
        EOG1 = contains(hdr.label, 'Eog');
        EKG1 = contains(hdr.label, 'ECG1');
        EKG2 = contains(hdr.label, 'ECG2');
        
        EEG = data(C3,:); %-  data(A2,:);
        EOGR = data(EOG1,:); %-  data(A1,:); 
        EOGL = data(EOG1,:); %-  data(A2,:); 
        EMG = data(EMG2,:); %-  data(EMG1,:);
        ECG = data(EKG2,:); %- data(EKG1,:);
        
        data = [EEG; EOGR; EOGL; EMG; ECG];
        
        % Index header
        idx = [find(C3),  find(EOG1), find(EOG1), find(EMG1), find(EKG1)];
        
        %  update hdr to only contain the derived signals
